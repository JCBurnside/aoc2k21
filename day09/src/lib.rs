...
impl Floor {
    ...
    fn get_adjacent(&self, point: (usize, usize)) -> [u8; 4] {
        [
            point
                .1
                .checked_sub(1)
                .map(|y| self.floor[point.0 + (y * self.size.0)])
                .unwrap_or(u8::MAX), //up
            point
                .0
                .checked_sub(1)
                .map(|x| self.floor[x + (point.1 * self.size.0)])
                .unwrap_or(u8::MAX), //left
            if point.0 == self.size.0 - 1 {
                u8::MAX
            } else {
                self.floor[(point.0 + 1) + (point.1 * self.size.0)]
            }, //right
            if point.1 == self.size.1 - 1 {
                u8::MAX
            } else {
                self.floor[point.0 + ((point.1 + 1) * self.size.0)]
            }, //down
        ]
    }
    ...
    fn get_adjacent_until_9(&mut self, point: (usize, usize)) -> Vec<u8> {
        ...
        .filter_map(|(change_x, change_y)| {
            if (change_x as i32 + point.0 as i32) < 0 || (change_y as i32 + point.1 as i32) < 0 {
                None
            } else if (change_x as i32 + point.0 as i32) >= self.size.0 as i32
                || (change_y as i32 + point.1 as i32) >= self.size.1 as i32
            {
                None
            } else {
                Some(self.get_adjacent_until_9((
                    (change_x as i32 + point.0 as i32) as usize,
                    (change_y as i32 + point.1 as i32) as usize,
                )))
            }
        })
        .flatten()
...
fn part1(floor: &Floor) -> usize {
    (0..floor.size.0)
        .cartesian_product(0..floor.size.1)
        .filter_map(|p| {
...
fn part2(mut floor: Floor) -> usize {
    (0..floor.size.0)
        .cartesian_product(0..floor.size.1)
        .map(|basin| floor.get_adjacent_until_9(basin))
        .map(|v| v.len())
        .sorted()
        .rev()
        .take(3)
