...
#[derive(
    parse_display::Display, parse_display::FromStr, Clone, Copy, Hash, PartialEq, Eq, Ord, Debug,
)]
#[display("{0},{1}")]
...
impl Line {
    ...
    fn get_line_points(&self) -> Vec<Point> {
        ...
        let change = (
            coerce_one(self.start.0, self.end.0),
            coerce_one(self.start.1, self.end.1),
        );
...
5,5 -> 8,2";
    ...
    fn line_test() {
        use super::Point;
        let parsed = super::parse(TEST);
        for (actual, expected) in parsed.into_iter().map(|line| line.get_line_points()).zip(
            vec![
                vec![
                    Point(0, 9),
                    Point(1, 9),
