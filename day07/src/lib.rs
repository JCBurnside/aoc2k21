...
fn part2(data:&[i32]) -> i32 {
    ...
    std::cmp::min(data.iter().map(|crab| (1i32..=(crab-mean.floor() as i32).abs()).sum::<i32>()).sum(),
    data.iter().map(|crab| (1i32..=(crab-mean.ceil() as i32).abs()).sum::<i32>()).sum())
}

fn parse(input : &str)->Vec<i32> {
    input.split(',').map(<i32 as FromStr>::from_str).map(Result::unwrap).collect()
}
